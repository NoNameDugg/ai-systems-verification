...
# host absolute paths, licensed-data identifiers, a third-party text, a local dev connection string.
# Exits non-zero (fails CI) if any forbidden string appears anywhere in the tree.
set -uo pipefail
ROOT="${1:-.}"

# Unambiguous leaks only — host abs-paths, licensed-data identifiers, a local dev connection string, a third-party text, real token prefixes.
